...
from private_space import Private_space
...
class MainPupilWindow(QMainWindow):
    ...
    def updateTable(self):
        cur = self.con.cursor()
        grade, name, author, query_type = self.comboBox.currentText(), self.lineEdit.text(), self.lineEdit_2.text(), self.comboBox_2.currentText()
        ...
        print(query)
        info = [(i[0], i[1]) for i in cur.execute(query).fetchall() if name.lower() in i[1].lower() and author.lower() in i[2].lower()]
    ...
    def private_space(self):
        self.private_info_window = Private_space(self.db, self.login)
...
class BookInfoWindow(QDialog):
    def __init__(self, main_window, book_id):
        ...
        name, author, grade, image, self.n = cur.execute(f'select name, author, grade, image, n from library where id = {book_id}').fetchone()
    ...
    def buttons_availability(self):
        ...
        books = str(cur.execute(f'select books from pupils where login = {self.main_window.login}').fetchone()[0]).split()
    ...
    def remove_book(self):
        book_id = self.lineEdit.text()
        list_books = [self.main_window.listWidget.item(i).text().split(';')[0] for i in range(self.main_window.listWidget.count())]
        self.main_window.listWidget.takeItem(list_books.index(book_id))
        self.buttons_availability()
...
if __name__ == '__main__':
    ...
    ex = MainPupilWindow(4, 'library.db')
    ex.show()
    sys.exit(app.exec_())
