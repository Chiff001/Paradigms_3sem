...
class Game():
    ...
    def show_score(self, n=1): # Показывает результат и ник игрока
        ...
        if n == 1: # Если игра не закончена результат выводится в левом верхнем углы
            rect.midtop = (65, self.y0 / 2 - 5)
            rect1.midtop = (self.screen_width - 45, self.y0 / 2 - 5)
            ...
            rect.midtop = ((self.screen_width + self.x0) / 2,
                             (self.screen_height + self.y0) / 2 + 15)
            self.screen.blit(surf, rect)
...
class Food(): # Еда
    ...
    def draw_food(self, screen, lvl):
        # Получаем рандомное число от 0 до 10, числа 0, Большая задача по Maps API. Часть №5, Список профессий, 6, 7, 8, 9 это обычное яблоко
        # Число 5 это золотое яблоко
        # Числа 3, 4 - серая сфера
        # Обычное яблоко: увеличивает скорость, прибавляет Большая задача по Maps API. Часть №5 балл
